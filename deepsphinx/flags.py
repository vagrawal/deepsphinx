...
def load_flags(mode = 'train'):
    ...
    flags.DEFINE_float('keep-prob', 0.75, 'Keep probability for dropout')
    flags.DEFINE_integer('max-output-len', 300, 'Maximum output length for Beam Search')
    flags.DEFINE_integer('rnn-size', 256, 'Size of LSTM')
    flags.DEFINE_integer('num-layers', 4, 'Number of LSTM Layers for encoding layer')
    flags.DEFINE_integer('num-decoding-layers', 1, 'Number of LSTM Layers for decoding layer')
    ...
    flags.DEFINE_integer('beam-width', 20, 'Beam width. Must be lesser than vocab size')
    flags.DEFINE_float('learning-rate-decay', 1.0, 'Learning rate decay')
    ...
    flags.DEFINE_integer('cutoff-range', 1000, 'Attention cutoff from previous mean range')
    ...
    flags.DEFINE_bool('use-conv-feat-att', True, 'Use convolutional features for attention')
    flags.DEFINE_integer('num-conv-layers', 0,
